...
### 성공 요인 (What Worked)
1. **단순명료한 프롬프트 설계**: 복잡한 조건 부여나 구체적인 가이드보다 직접적인 지시문("자동차 아니면 0, 맞으면 1")이 오히려 최고 정확도를 기록했습니다.
2. **구문(Syntax) 제약에 따른 영향**: 마침표와 쉼표를 제거했을 때 성능이 미세하게 향상되는 패턴을 발견했습니다. (토큰화 과정에서의 불필요한 맥락 제약 해소)
3. **언어 의존성 파악**: 동일한 의미의 프롬프트라도 영어보다 한국어 프롬프트가 모델(GPT-4o-mini)에서 평균 6.5% 더 안정적이고 우수한 한국어 뉴스 분류 성능을 보였습니다. 

### 실패 사례와 교훈 (What Failed)
1. **조건문 생략의 역효과 ("자동차 1")**: 극단적으로 점수 페널티를 줄이기 위해 "자동차 1"처럼 맥락과 조건(Condition)을 생략한 프롬프트는 10.9%의 처참한 정확도를 기록했습니다. 모델에 최소한의 행동 지시(Instruction)는 필수적입니다.
2. **체인-오브-사고(CoT)의 오작동**: "주제를 파악하고...", "단계별로 생각해봐" 등의 추론 강화 기법은 오히려 단순 이진 분류 태스크에서 출력 포맷을 훼손하거나 과적합을 유발했습니다. 복잡한 추론 가이드보다는 명확한 출력 제약이 더 중요합니다.
3. **주관적 기준의 모호성**: "자동차 관련 내용이 '핵심'이면"과 같은 모호한 표현의 도입은 모델마다 일관성 붕괴를 초래해 가장 큰 성능 하락을 가져왔습니다.
